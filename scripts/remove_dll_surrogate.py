...
64-bit Python用に追加したDLL Surrogate設定を削除します。
...
def delete_registry_key(root, subkey):
    ...
    try:
        ...
        return False, "キーが存在しません"
    except OSError as e:
        if e.winerror == 5:
            return False, "管理者権限が必要です"
        elif e.winerror == 145:
            return False, "キーが空ではありません（サブキーまたは値が存在）"
        return False, str(e)
    except Exception as e:
        return False, str(e)
...
def remove_dll_surrogate(name, clsid, force=False):
    ...
    # 削除対象
    # 注: AppIDキー自体は他のソフトウェアが使用する可能性があるため、
    #     値のみを削除し、キーは残す
    targets = [
        # (path, value_name, description)
        (f"CLSID\\{clsid}", "AppID", "64bit CLSID AppID"),
        (f"AppID\\{clsid}", "DllSurrogate", "64bit AppID DllSurrogate"),
        (f"Wow6432Node\\CLSID\\{clsid}", "AppID", "32bit CLSID AppID"),
        (f"Wow6432Node\\AppID\\{clsid}", "DllSurrogate", "32bit AppID DllSurrogate"),
...
"""
    ...
    if args.force:
        ...
        print("\n実際に削除するには:")
        print("  python scripts/remove_dll_surrogate.py --force")
