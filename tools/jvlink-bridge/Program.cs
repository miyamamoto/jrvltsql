...
class BridgeForm : Form
{
    ...
    private void HandleClose()
    {
        if (_jvlink != null && _isOpen)
        {
            ...
            try
            {
                code = _jvlink.JVClose();
            }
            catch (Exception)
            {
                WriteResponse(new { status = "error", code = -9999, error = "JVClose invocation failed" });
                return;
            }
            if (code != 0)
            {
                WriteResponse(new { status = "error", code, error = "JVClose failed" });
                return;
            }
            _isOpen = false;
        }
        WriteResponse(new { status = "ok", code = 0 });
    }
