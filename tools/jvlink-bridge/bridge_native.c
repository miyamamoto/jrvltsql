...
static int CallMethodNoArgs(IDispatch* disp, const wchar_t* method) {
    DISPID id;
    if (FAILED(GetDispId(disp, method, &id))) return -9999;
...
static int CallJVRead(char* buff, int buff_size, char* filename, int fn_size, int* payload_len) {
    *payload_len = 0;
    DISPID id;
    ...
    if (bBuff) {
        if (code > 0) {
            ...
            int copy_len = 0;
            int raw_byte_bstr = bstr_len >= (UINT)code;

            /*
             * JV-Link can expose JV-Data in either of two BSTR forms:
             * raw bytes zero-extended into UTF-16 code units, or decoded
             * Unicode text. Preserve the former byte-for-byte. Convert the
             * latter explicitly back to the provider's CP932 bytes.
             */
            if (raw_byte_bstr) {
            ...
            if (raw_byte_bstr && code <= buff_size) {
                copy_len = code;
                for (int i = 0; i < copy_len; i++) {
                    buff[i] = (char)(bBuff[i] & 0x00ff);
                }
            } else {
                int encoded_len = WideCharToMultiByte(
                    932, 0, bBuff, (int)bstr_len, NULL, 0, NULL, NULL
                );
                if (encoded_len > 0 && encoded_len <= buff_size) {
                    copy_len = WideCharToMultiByte(
                        932, 0, bBuff, (int)bstr_len,
                        buff, buff_size, NULL, NULL
                    );
                }
            }
            buff[copy_len] = '\0';
...
// Call JVClose and preserve its official return code / invocation failure.
static int CallJVClose(void) {
    return CallMethodNoArgs(g_jvlink, L"JVClose");
}
...
int main(int argc, char* argv[]) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if (strcmp(cmd, "init") == 0) {
        ...
        else if (strcmp(cmd, "setuiproperties") == 0) {
            HRESULT hr = EnsureJVLink();
            if (FAILED(hr) || !g_jvlink) {
                json_response("{\"status\":\"error\",\"error\":\"CoCreateInstance failed\",\"hr\":\"0x%08X\"}", (unsigned)hr);
                free(cmd); continue;
            }
            int code = CallMethodNoArgs(g_jvlink, L"JVSetUIProperties");
            json_response("{\"status\":\"%s\",\"code\":%d}", code == 0 ? "ok" : "error", code);
        }
            ...
            // Official contract: -1 (no data) still requires JVClose.
            if (code >= -1) g_is_open = 1;
            ...
            char filename[512] = "";
            int payload_len = 0;
            int code = CallJVRead(buff, size, filename, sizeof(filename), &payload_len);
            if (code > 0) {
                /*
                 * JVRead's positive return value is the fixed-width record
                 * length in bytes. The BSTR can contain embedded NUL code
                 * units for blank fixed-width fields, so strlen() truncates
                 * valid RA/SE records at the first blank field.
                 */
                int dlen = payload_len;
            ...
            int code = 0;
            if (g_jvlink && g_is_open) code = CallJVClose();
            if (code == 0) g_is_open = 0;
            json_response("{\"status\":\"%s\",\"code\":%d%s}",
                         code == 0 ? "ok" : "error", code,
                         code == 0 ? "" : ",\"error\":\"JVClose failed\"");
        }
