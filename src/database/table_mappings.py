...
and jrvltsql table names, covering all 38 supported record types for JRA.
...
Helper Functions:
- get_table_name_for_source(): Get table name based on record type
- get_jravan_table_name(): Get table from JRA-VAN standard name
...
JRAVAN_TO_JLTSQL: Dict[str, str] = {
    ...
    "SAKURO": "NL_HC",        # 坂路調教 (legacy alias)
    "AVOIDENCE": "NL_AV",     # 出走取消・競走除外 (legacy alias)
...
def get_table_name_for_source(record_type: str, source: str = "jra") -> Optional[str]:
    ...
    Args:
        ...
        source: Data source (only "jra" is supported)
...
def get_jravan_table_name(standard_name: str, source: str = "jra") -> Optional[str]:
    ...
    Returns:
        jrvltsql table name, or None if not found
...
def get_record_type_from_table(table_name: str) -> Optional[str]:
    """Get record type code from table name.
