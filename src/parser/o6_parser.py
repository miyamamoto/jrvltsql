    ...
    def parse(self, data: bytes) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if len(data) < self.RECORD_LENGTH:
                self.logger.warning(
                    f"O6レコード長不足: expected={self.RECORD_LENGTH}, actual={len(data)}"
                )
            ...
            # 13. 発売フラグ　3連単 (位置:40, 長さ:1)
            result["SanrentanFlag"] = self.decode_field(data[39:40])

            # 14. <3連単オッズ> (位置:41, 長さ:0)
            ...
            # 16. 　　オッズ (位置:47, 長さ:7)
            result["Odds"] = self.decode_field(data[46:53])

            # 17. 　　人気順 (位置:54, 長さ:4)
            result["Ninki"] = self.decode_field(data[53:57])

            # 18. 3連単票数合計 (位置:58, 長さ:11)
            result["Vote"] = self.decode_field(data[57:68])

            # 19. レコード区切 (位置:69, 長さ:2)
            # result["Field19"] = self.decode_field(data[68:70])
