...
| `tools/export_timeseries_csv.py` | 保存済みオッズの確認 | `TS_O1` / `TS_O2` や `TS_SOKUHO_O*` を確認用 CSV として出力します。 | JRA-VAN からの取得は行いません。 |
...
速報オッズは公式長期時系列の `TS_O1` / `TS_O2` とは分けて、
`TS_SOKUHO_O1`〜`TS_SOKUHO_O6` に保存します。
