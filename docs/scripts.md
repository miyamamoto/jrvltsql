...
| --- | --- | --- | --- |
| `quickstart.bat` | まず手元で SQLite DB を作るとき | 通常セットアップ、データ取得、DB 検証を行います。必要なら PostgreSQL 時系列オッズ投入へ続けます。 | これ単体では PostgreSQL 常時運用は確定しません。 |
...
`jltsql realtime odds-sokuho-timeseries` を別途実行してください。
