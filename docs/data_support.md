...
予備をNULLへ落としません。速報では着順確定前の人気が空欄になり得るため、空欄を
正常値として扱います。この挙動は公式スタッフの
[説明](https://developer.jra-van.jp/t/topic/304)と一致します。また、provider flagを
理由に本文を間引かず完全に保存する方針は、サンプル修正に関する公式スタッフの
[説明](https://developer.jra-van.jp/t/topic/64)に従います。

2004-03-02の仕様変更はレコード長を変えず、位置604〜717の予備領域を三連単関連へ
転用しました。三連単発売開始日の2004-08-14より前のraceでは、この114バイトを
...
旧nullable key、主キーなし、予備2・3件目の列欠落、型・容量不一致、追加の
