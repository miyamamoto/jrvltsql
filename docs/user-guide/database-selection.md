...
JRVLTSQLは**SQLite**（デフォルト）と**PostgreSQL**をサポートしています。
...
|------|------|
| **用途** | マルチユーザー・サーバー環境 |
| **セットアップ** | PostgreSQLサーバーのインストールが必要 |
| **同時接続** | 複数ユーザー対応 |
| **大規模データ** | 高速処理 |
...
### Q: PostgreSQLを使う場合の注意点は？

A: JV-Link COM DLLへのアクセスに32-bit Pythonが必要です。PostgreSQLのドライバ（pg8000）は32-bit Python環境でも動作します。
