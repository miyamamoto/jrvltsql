...
555 byte の現行 SE レコード（`src/parser/se_parser.py`）末尾には
**マイニング（DM）ブロック**がある:

| # | フィールド | 位置 |
|---|---|---|
| 63 | DMKubun | 537 |
| 64 | DMTime（予想走破タイム） | 538 |
| 65 | DMGosaP | 543 |
| 66 | DMGosaM | 547 |
| 67 | DMJyuni（予想順位） | 551 |
| 68 | **KyakusituKubun（今回レース脚質判定）** | 553 |

旧 463 byte レイアウトの位置は無効である。現行 parser は相手馬情報3件を含む
公式 555 byte レイアウトのみを正本とする。詳細は
`docs/crawler_audit_04_se_layout.md` を参照すること。
