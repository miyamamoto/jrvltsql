...
1. **データベース選択**: SQLite（デフォルト）/ PostgreSQL
2. **サービスキー確認**: JV-Link の接続チェック
3. **セットアップモード選択**: 簡易 / 標準 / フル
4. **データ範囲指定**: 過去何年分のデータを取得するか
5. **オプション設定**: 時系列オッズ、当日レース情報、自動更新サービス
...
JV-Linkでは`option=4`でセットアップ（全データ取得）、`option=1`で差分取得を行います。11ヶ月以上前のデータはquickstartが自動的に`option=4`を選択します。
