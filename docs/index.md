...
| 開催週の速報オッズ | `TS_SOKUHO_O1`〜`TS_SOKUHO_O6` | 全賭式対応。ただし JRA-VAN 側の保持は約1週間です。 |
| 日次同期 | `daily_sync.bat` | 既定では直近通常データ、公式時系列、開催週速報を更新します。通常データだけなら `--no-timeseries --no-realtime` を指定します。 |
