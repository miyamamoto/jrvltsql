...
- 不成立・特払・返還の 3 つの券種 flag 配列は、6 件目が公式予備です。公式が
  ...
  文字列で保持し、空白も SQL `NULL` へ置換しません。返還馬番 28 件、返還枠番
  8 件、返還同枠 8 件は全要素が
...
- 旧 nullable key、主キーなし、予備 2・3 件目の列欠落、予備列（3 つの flag
  配列の 6 件目を含む）の数値型、
...
- 各フラグは未設定時の公式初期値も `0` なので、非削除レコードでは常に `0` か
  ...
  各区画に収まる限り内容を問わずそのまま保持します。空白も SQL `NULL` へ
  置換せず、空文字として保持します。
