...
UmaConn は、**初回データダウンロードを API 経由で完了する機能を提供していません**。初回セットアップは公式アプリケーションを通じて手動で実行する必要があります。

### 解決策

1. スタートメニュー → 地方競馬DATA → セットアップ
2. または直接実行: `C:\UmaConn\chiho.k-ba\data\UmaConn設定.exe`

初回セットアップ完了後は、API経由での差分データ取得が正常に動作します。
